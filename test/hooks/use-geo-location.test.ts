...
describe("useGeoLocation", () => {
  it("should return default values initially", () => {
    ...
    vi.mocked(browserGeo.isBrowserGeolocationAllowed).mockResolvedValue(false);
  ...
  it("should have correct status type", async () => {
    ...
    vi.mocked(browserGeo.isBrowserGeolocationAllowed).mockResolvedValue(true);

    const { result } = renderHook(() => useGeoLocation());

    await waitFor(() => {
      expect([GeoLocationStatus.Loading, GeoLocationStatus.Success]).toContain(result.current.status);
    });
  });
});
