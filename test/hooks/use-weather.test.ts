...
describe('useWeather', () => {
  it('should return default values when location is provided', () => {
    ...
    vi.mocked(googleWeatherHooks.useCurrentConditions).mockReturnValue({ data: null, isError: false, isLoading: false } as any)
    vi.mocked(googleGeocodingHooks.useReverseGeocode).mockReturnValue({ data: null, isLoading: false } as any)
    vi.mocked(googleWeatherHooks.useDailyForecast).mockReturnValue({ data: null, isLoading: false } as any)
  ...
  it('should handle loading state correctly', () => {
    vi.mocked(UnitsSystemContext.useUnitsSystem).mockReturnValue({ unitsSystem: 'METRIC', setUnitsSystem: vi.fn() })
    vi.mocked(googleWeatherHooks.useCurrentConditions).mockReturnValue({ data: null, isError: false, isLoading: true } as any)
    vi.mocked(googleGeocodingHooks.useReverseGeocode).mockReturnValue({ data: null, isLoading: false } as any)
    vi.mocked(googleWeatherHooks.useDailyForecast).mockReturnValue({ data: null, isLoading: false } as any)
